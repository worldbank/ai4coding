...
- **Customize** your Positron assistant with instructions, prompt files, and
  skills.
...
We will use the same examples as in the self-study exercises. Please note that
...
- **WB caveat**: MCP extensions are not available via the WB Software Center;
  servers must be configured manually in `.vscode/mcp.json`.
- Key distinction: MCP is a protocol exposing tools + resources + prompts;
  tools are individual callable functions within the IDE.
...
Understanding agents, subagents, and the agent loop. Customizing AI behavior
