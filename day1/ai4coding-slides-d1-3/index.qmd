...
- 💻 IDE: Integrated Development Environment, e.g., Positron, RStudio, VS Code, and more.
...
- {{< fa solid robot >}} **Positron Assistant** works in the IDE as a middleman that connects the user, IDE, and cloud-based AI services.

- {{< fa brands github >}} **GitHub Copilot** is a cloud-based AI orchestrator: coordinates the assistant and LLMs, submits prompts to the LLMs, and pre-processes responses before sending them back to the assistant.
...
## Context saturation {.small-slide}

- ⚠️ When the context window fills up, old context is **pushed out**, causing [information loss]{.fg} and reduced quality.

- 🗜️ Assistants may **compress context** via LLM summarization — but this is [suboptimal]{.warning}: key details can be lost.
...
[**Be cautious with sensitive data. Review all suggestions. Ask the LLM to explain its suggestions.** [Follow recommendations to safeguard data](/methods/safeguard-data.qmd)]{.warning}
...
::: columns
::: {.column width="48%"}
![LLM training illustration](images/3b1b-training.jpg){width="100%" .fragment .center}
:::
::: {.column width="2%"}
:::
::: {.column width="48%"}
![Backpropagation illustration](images/3b1b-backpropogation.jpg){width="100%" .fragment .center}
:::
:::
...
![Next-token prediction example](images/3b1b-predictions.jpg){ width="100%" .center}
...
![Context-dependent LLM prediction](images/llm-prediction-2.png){width="100%" .center}
...
![LLM inference diagram](images/inference.png){width="80%" .center}
...
[Hallucinations]{.fg} occur when an LLM generates text that is fluent, confident, and **wrong**. Common causes: insufficient or irrelevant context, training data biases or gaps, and over-generalization of learned patterns.
