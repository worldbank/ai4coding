...
- 🤖 AI assistants:
  ...
  -  Cloud-based providerd: GitHub Copilot, MAI, Anmthropic Claude API, ...
...
is the [workspace for coding and executing data analysis]{.fg}
...
- 💻 **Code, data, and analysis** stay inside the IDE on user's computer.
- {{< fa solid robot >}} **Positron Assistant** works in the IDE as a middleman that connects user, IDE, and cloud-based AI services.

- {{< fa brands github >}} **GitHub Copilot** is a cloud-based AI orchestrator: co-steers assistant and LLMs, submits prompts to the LLMs, pre-processes responses before sending it back to the assistant.


- 🔢 **The LLM**:

  - ingesting **prompt as text**,
  - converts it **into tokens**,
  - **embeds tokens as numeric vectors**,
  - **performs inference** to generate tokenized response, decodes tokens, and
...
⚠️ LLMs have a max context window (64k–2M tokens) — exceeding it leads to information loss.
...
- LLMs respond with a distribution of likely next words and then choose one following an algorithm.
...
- Process of responses generation based on the input prompt, which contains all the context.
...
[Hallucinations]{.fg} happen when **LLM does not have sufficient context** and
produces text that is fluent, confident, and **wrong**.

::: {.columns}
::: {.column width="50%"}
Hallucinations patterns

  - Fake citations
  - Wrong but close facts
  - Entity blending
  - Overconfident extrapolation
    - correctly reasoning partway through a problem, then guessing the rest as
      if it were known.
  - Recency failures
...
- Use fresh agents to fact-check and verify LLMs results re-reading sources.
:::
:::
...
- 2blue1brown's
  ...
  - or [Youtube playlis](https://www.youtube.com/playlist?list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi)
