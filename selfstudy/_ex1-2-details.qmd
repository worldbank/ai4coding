...
Whatch out for the AI trying to use command line tools or
power-shell tools to the folder and list all files in it.
...
The ai should produce a Stata do-file but it may or may not run it.
Therefore, you should then see how to act further. I usually ask AI
to run the code and check the output iteratively. If data is sensitive,
you may want to run it yourself. To ask AI to run it, you can use a prompt like this:
...
::: {.callout-warning}
## WATCH OUT FOR AI TRYING TO INVENT HOW TO RUN STATA

The problem is that AI may not be aware of the Stata console and may try to
invent a way to run the code itself. For example, AI could write a Python script
that passes a do file into Stata and executes it. **We must prevent this** because
it is not only unnecessary but also a potential security risk.

To make AI run code in our controlled environment we must indicate to it what
`tool` to use! (we will cover tools in Day 2). For now, simply answer with the
following prompt if AI tries to invent the way of running Stata code itself.
:::
...
`#executeCode` is a special instruction that tells the AI to run the code
in the console, if it has access to one. If it does not have access, it
should say so and you can run the code yourself.

::: {.callout-tip }
Sometimes it may take Stata long to execute code.
The stata console may ran into the `ERROR: Execution timed out after 600s`.

Go to Positron settings `Ctrl + ,` and find `positron.stata.runFileTimeout`
increase the `Execution timeout` to a higher value, e.g., 1200s or 1800s.
:::
...
It is recommended to use AI at this stage to document the data even further,
creating a synthesis of what this data is about and what are the key similarities
and differences between the two waves. Perhaps, it makes sence to use existing
context window to write such readme as it has all preloaded information about
the data structure and metadata. However, if you want to use a fresh agent,
that may also give additional insights free of some learning biases.
Try both and see which one gives better results! Use the following prompt:
