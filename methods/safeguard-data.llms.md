...
4.  follow organizational policies and legal requirements for responsible AI use ([ai/](https://ai.worldbank.org/)).
...
In general, **AI does not load raw data into context as is**. However, as it develops code to understand the data, it can potentially propose doing things that lead to data exposure. Based on the literature,[^1] the most common channels of data exposure are:
...
>
> For guidance on handling PII in research projects, see the DIME Data Handbook: [Chapter 5 — Processing confidential data](https://worldbank.github.io/dime-data-handbook/processing.html#processing-confidential-data) and [Chapter 4 — Handling data securely](https://worldbank.github.io/dime-data-handbook/acquisition.html#handling-data-securely).
...
>
> 2.  Add the following instruction, replacing the placeholder paths with the actual folders and files that contain sensitive data in your project.
>
>     ``` numberSource
...
> **NOTE:**
>
> You can add this text into the System instructions for your AI tool (see [Instruct AI to NOT read specific files for setup steps](#instruct-ai-not-read-files)), so that it is applied to every chat session with AI in Positron in your project. Follow the same steps as [in the previous section](#instruct-ai-not-read-files) to add the necessary instructions to the `.github/copilot-instructions.md` and `claude.md` files.
...
This approach aligns with the de-identification workflow recommended in the DIME Data Handbook; see [Chapter 5 — Implementing de-identification](https://worldbank.github.io/dime-data-handbook/processing.html#implementing-de-identification).
...
2.  Execute this code on your local machine, in software where you can control the process and ensure that metadata files are generated and saved in the working folder, while raw data files remain secure outside of it.

3.  When you continue working with your project, where only metadata is present, tell AI that:
...
The techniques below are widely used in SDC and are described in detail in the [Data Privacy Handbook — De-identification techniques](https://utrechtuniversity.github.io/dataprivacyhandbook/deidentification-techniques.html) and the [IHSN SDC Theory Guide](https://sdctheory.readthedocs.io/en/latest/):
...
- [**k-anonymity**](https://en.wikipedia.org/wiki/K-anonymity): each record is indistinguishable from at least *k − 1* other records on the quasi-identifiers.
...
See [Data Privacy Handbook — k-anonymity, l-diversity and t-closeness](https://utrechtuniversity.github.io/dataprivacyhandbook/k-l-t-anonymity.html) for an accessible introduction.

#### Available tools by language

## R

- **sdcMicro** — The reference R package for Statistical Disclosure Control, developed at Statistics Austria. Implements risk estimation, local suppression, recoding, microaggregation, noise addition, PRAM, and record swapping. Includes a Shiny GUI (`sdcApp`) and AI-assisted anonymization features. See Templ, Kowarik & Meindl (2015) and the [AI-Assisted SDC vignette](https://cran.r-project.org/web/packages/sdcMicro/vignettes/ai_assisted_anonymization.html). CRAN: <https://CRAN.R-project.org/package=sdcMicro>
...
There are currently no established Stata packages for automated SDC. Practitioners typically implement anonymization manually (recoding, top-coding, noise addition) or call R/Python via Stata’s interoperability features (`rsource`, `python:`). The [IHSN Practice Guide](https://sdcpractice.readthedocs.io/en/latest/) provides worked examples that can be adapted to Stata workflows.
...
[^3]: Bjarkefur, Cardoso de Andrade, Daniels & Jones. *Development Research in Practice: The DIME Analytics Data Handbook* — Chapter 5: Processing confidential data. <https://worldbank.github.io/dime-data-handbook/processing.html#processing-confidential-data>

[^4]: Utrecht University. *Data Privacy Handbook — Pseudonymisation & Anonymisation*. <https://utrechtuniversity.github.io/dataprivacyhandbook/pseudonymisation-anonymisation.html>

[^5]: Utrecht University. *Data Privacy Handbook — Statistical Approaches to De-identification*. <https://utrechtuniversity.github.io/dataprivacyhandbook/statistical-privacy.html>
